...
namespace Control_Station
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            raspberryPiHostName = "raspberrypirover"; // Raspberry Pi'nin adı (bazen raspberrypi.local gibi)

            // Raspberry Pi'nin adını kullanarak IP adresini çözümleme
            IPAddress[] ipAddresses = Dns.GetHostAddresses(raspberryPiHostName);

            foreach (IPAddress ipAddress in ipAddresses)
            {
                if (ipAddress.AddressFamily == AddressFamily.InterNetwork) // IPv4 adresi
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (button2.Text == "Play Stream")
            {
                string theURL = "http://" + raspberryPiHostName + ":8080";
                axWindowsMediaPlayer1.URL = theURL;
                axWindowsMediaPlayer1.Ctlcontrols.play();
                button2.Text = "Stop Stream";
            }
            else if (button2.Text == "Stop Stream")
            {
                string theURL = "http://" + raspberryPiHostName + ":8080";
                axWindowsMediaPlayer1.URL = theURL;
                axWindowsMediaPlayer1.Ctlcontrols.stop();
                button2.Text = "Play Stream";
            }
